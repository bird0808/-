...
async def asynGetChapterUrl(url):
    async with semaphore:
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as result:
                ...
                for i in tree.xpath("//dd//a/@href"):
                    urls.append(list_url+i)
                return urls
async def asynGetAllCatalogueUrl():
    # 这个函数通过异步获取所有章节的链接，并返回一个列表
    urls = [f'{list_url}index_{index}.html' for index in range(1, catalogue_max_pages + 1)]
    task=[]
    for url in urls:
        task.append(asyncio.create_task(asynGetChapterUrl(url)))
    results=await asyncio.gather(*task)
    data=results
    data1 = []
    for i in data:
...
async def asyncGetAllNovel(urls):
    task = []
    for i in urls:
        task.append(asyncio.create_task(getNovel(i)))
    # gather方法会根据task的顺序，返回有序的结果，asyncio.wait()返回的则是无序的结果
...
async def main():
    ...
    urls = await asynGetAllCatalogueUrl()
    
    data = await asyncGetAllNovel(urls)
